...
\begin{tikzpicture}[auto]
    ...
    \node [block, below left of=inloop_cond, minimum width=4cm, node distance=3.0cm] (inloop_yes) {\hbox{$pow = pow/2$} \hbox{$result = 2 \cdot result$}};
    \path [line] (inloop_cond) -| node[near start, above]  {да}(inloop_yes);

    \node [block, below right of=inloop_cond, minimum width=4cm, node distance=3.0cm] (inloop_no) {\hbox{$pow = pow-1$} \hbox{$result = result \cdot x_1$}};
